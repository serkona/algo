...
Чтение через mmap на флеймграфе не выделяется отдельным фреймом — page fault'ы обрабатываются ядром ОС и не видны async-profiler'у в Java-стеке.
...
- **Page size создаёт трейдофф между чтением и записью.** Внутри каждого бакета записи хранятся последовательно, `get()` выполняет линейный проход по всем записям до нахождения ключа — индекса внутри бакета нет.
  - **GET деградирует при крупных страницах** (1 KB: 8.7M ops/s → 16 KB: 3.4M) — бо́льшая страница вмещает больше записей до split, поэтому линейный проход длиннее.
  - **PUT растёт с увеличением страницы** (1 KB: 7.2K → 16 KB: 138K ops/s) — при мелких страницах частые split-операции (перечитывание записей, перераспределение, flush метаданных) замедляют вставку.
...
- Offset-индекс внутри бакета — массив `[hash:4][offset:4]` для каждой записи, отсортированный по hash; поиск по бинарному поиску вместо линейного прохода — устранит деградацию GET при крупных страницах
- Батчевание `saveMeta()`/`force()` — сейчас каждый split делает синхронную запись метаданных; отложенный flush снизит I/O при массовых вставках
...
GC = 0 на всех lookup-операциях. После вынесения генерации случайных ключей в @Setup аллокации getMissing исчезли.
...
- Lookup ~0.43 us независимо от размера — O(1). Лёгкий рост (0.422 → 0.443) при увеличении таблицы от 1K до 45K — влияние cache locality при случайном доступе к большей таблице.
...
- B/op ~7.1K на add (claims) и ~11.0K (corpus) — shingling + сигнатура. После перехода на fastutil (Long2ObjectOpenHashMap для band-таблиц) аллокации снизились.
