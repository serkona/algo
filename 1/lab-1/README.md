...
- Батчевание `saveMeta()`/`force()` — сейчас каждый split делает синхронную запись метаданных; отложенный flush снизит I/O при массовых вставках (видно нап профиле)
- Перед каждым split убирать tombstones, сдвигая живые записи - это позволит реже сплитить  
- Bloom filter перед сканированием - ускоримся при обращении по несуществующему ключу (фильтр явно вернет это, без необходимости обхода всех бакетов)
...
- Rolling hash для shingling 
Для каждой позиции n-граммы хэш считается каждый раз заново - это несколько итераций (в зависимости от shingleSize).
...
- Считать MinHash одновременно с генерацией шинглов — -50% памяти. Сейчас отдельным проходом считается шинглы Set<Long>, а потом по этому множеству считается minHash.
